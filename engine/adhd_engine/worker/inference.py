"""ADHD real-time inference module.

Migrated from ``model/eye_tracker/adhd_inference.py`` for the engine refactor.
The model files in ``model/adhd_pupil_classifier/saved_models/`` are NOT
modified — they are loaded read-only via ``adhd_engine.config.SAVED_MODELS_DIR``.

Behavioural changes vs the original module:

1. **`pupil_slope` unit fix** — see plan §1.5.1.
   The training pipeline (``adhd_classifier.py:165``) computes slope at 1000 Hz
   so its unit is ``Δ(% baseline) / sample = Δ(%) / ms``. The original
   inference computed slope at 30 Hz with the same ``np.arange(slope_len)``
   x-axis, giving ``Δ(%) / 33.3 ms`` — i.e. values 33× larger than training.
   After the trained ``StandardScaler`` standardised slope using the training
   distribution, the inference value was effectively garbage. We now multiply
   by ``(fps / 1000.0)`` to convert back to the training unit. The same fix is
   applied to ``pupil_slope_var`` for consistency (it's not in the top-12
   selected features but is still computed as part of the 27-feature vector).

2. No other numerical changes. All other features (``max_peak``,
   ``mean_change``, ``peak_latency``, ``auc``, ``late_minus_early``, etc.) are
   already sample-rate-invariant by construction (relative-change
   normalisation + length-normalised aggregations).
"""
...
def extract_features(trial_results, fps=30):
    ...
    for t in trial_results:
        ...
        if slope_len > 5:
            ...
            # Plan §1.5.1 — convert from "Δ%/sample @ webcam fps" to
            # "Δ%/sample @ 1000 Hz" so the value matches what the trained
            # StandardScaler expects. At fps=30 this divides by ~33.3.
...
def predict_adhd(features, model_dir: Optional[Union[str, Path]] = None):
    """Run the trained Random Forest classifier on a 27-feature vector.

    Args:
        features: dict from :func:`extract_features` (27 keys).
        model_dir: optional override for the saved-models directory. Defaults
            to the vendored ``model/adhd_pupil_classifier/saved_models`` via
            :data:`adhd_engine.config.SAVED_MODELS_DIR`.

    Returns:
        dict with keys ``prediction``, ``adhd_probability``,
        ``control_probability``, ``risk_level``, ``feature_importance``,
        ``feature_values``, ``model_info``.
    """
    ...
    feat_cfg = joblib.load(model_dir / 'feature_config.joblib')
